...
data = pd.read_csv("C:\\Users\\PC\\Downloads\\Assets Data\\EURUSD_Candlestick_5_M_ASK_01.01.2023-04.01.2025.csv")
...
def calculate_return_and_mdd(entry_time, entry_price, trade_type, exit_price, data):
    ...
    exit_candle = trade_data.iloc[1:][(trade_data['Low'] <= exit_price) & (trade_data['High'] >= exit_price)]
...
def find_and_plot(trade_date_str, trade_type, entry_price, exit_price, risk, data):
    ...
    plt.scatter(match_time, entry_price, color="Green", label ="Entry True", linewidths=5)
    plt.scatter(gmt_entry_time, entry_price, marker='x', color="Orange", label ="Entry Approx", linewidths=1)
    plt.scatter(exit_time, exit_price, color="Red", label ="Exit True", linewidths=5)
...
trade_date = "25/04/2024 12:00:00"
trade_type = "short"
entry_price = 1.07386
exit_price = 1.07159
